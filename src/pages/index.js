	...
	const message_callback = ( message_json ) =>
	{
		...
		if( message_json.state )
		{
			if( message_json.state.reported.pressure )
			{
				...
				newDataSet.push( { x: ( message_json.timestamp * 1000 ), y: message_json.state.reported.pressure } );
				...
				<Card variant="outlined" style={ { paddingBottom: 10 } }>
					<CardContent>
						<Paper style={ { padding: 10, width: 600 } }>
							<Typography variant="h6" gutterBottom component="div">Pressure Monitor</Typography>
							<Divider/>
							<Grid container spacing={ 2 } style={ { marginTop: 0, marginBottom: 0 } }>
								<Grid container item md={ 6 }>
									<Grid item sm={ 6 }>
										<Typography variant="body2" gutterBottom style={ { height: '20px', color: PRIMARY_COLOR, fontWeight: 'bolder' } }>Time:</Typography>
									</Grid>
									<Grid item sm={ 6 }>
										<Typography variant="body2" gutterBottom style={ { height: '20px' } }>{ emptyStringIfEmpty( dateTimeFormatter( time ) ) }</Typography>
									</Grid>
								</Grid>
								<Grid container item md={ 6 }>
									<Grid item sm={ 6 }>
										<Typography variant="body2" gutterBottom style={ { height: '20px', color: PRIMARY_COLOR, fontWeight: 'bolder' } }>Pressure:</Typography>
									</Grid>
									<Grid item sm={ 6 }>
										<Typography variant="body2" gutterBottom style={ { height: '20px' } }>{ emptyStringIfEmpty( pressure ) }</Typography>
									</Grid>
								</Grid>
